...
import (
	...
	"github.com/ROCm/device-metrics-exporter/pkg/k8s"
	"github.com/ROCm/device-metrics-exporter/pkg/slurm"
	...
	"github.com/ROCm/device-metrics-exporter/pkg/amdgpu/parserutil"
...
func (ga *GPUAgentClient) ListWorkloads() (map[string]interface{}, error) {
...
func (ga *GPUAgentClient) getWorkloadInfo(wls map[string]interface{}, gpu *amdgpu.GPU) (podInfo k8s.PodResourceInfo, jobInfo slurm.JobInfo) {
	if ga.isKubernetes {
		if wl := wls[strings.ToLower(gpu.Status.PCIeStatus.PCIeBusId)]; wl != nil {
			podInfo = wl.(k8s.PodResourceInfo)
			return
		}
	} else {
		if wl := wls[fmt.Sprintf("%v", gpu.Status.Index)]; wl != nil {
			jobInfo = wl.(slurm.JobInfo)
			return
		}
	}
	return
...
func (ga *GPUAgentClient) populateLabelsFromGPU(wls map[string]interface{}, gpu *amdgpu.GPU) map[string]string {
	...
	podInfo, jobInfo := ga.getWorkloadInfo(wls, gpu)
